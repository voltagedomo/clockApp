...
// Set the canvas dimensions and center
canvas.width = 300;
canvas.height = 300;
...
function getTime() {
  ...
  setInterval(updateClock(selectedHours, selectedMinutes), 1000);
}

getTime();

let selectorStopwatch = document.getElementById("selector-stopwatch");
let selectorTimer = document.getElementById("selector-timer");
let selectorClock = document.getElementById("selector-clocks");
...
selectorStopwatch.addEventListener("click", hideCanvas);
selectorTimer.addEventListener("click", hideCanvas);
selectorClock.addEventListener("click", showCanvas);
